...
exports.forgotPassword = handleAsync(async (req, res, next) => {
    ...
    if (!user) {
        return next(new ErrorResponse("There is not user with this email", 404))
    }
    ...
    try {
        ...
        return next(new ErrorResponse("There was an error sending email!", 500))
    }
});
...
exports.tokenProtect = handleAsync(async (req, res, next) => {
...
exports.restrictTo = (...userRoles) => {
    return (req, res, next) => {
        if (!userRoles.includes(req.user.role)) {
            return next(new ErrorResponse("You do not have permission to perform this action!", 401))
        }
        next()
    }
};
